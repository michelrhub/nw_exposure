-- ============================================================================
-- Network Exposure - Private/Non-Public Scope Query Pack (Oracle Autonomous Database)
--
-- Focus: exposure analysis outside public-subnet scope.
...
-- ============================================================================
-- In-Use Insecure Security Lists in Private/Non-Public Scope
-- Returns insecure security list rules (ALL ports - TCP/22 - TCP/3389) effectively used by subnets outside public-scope definition.
-- ============================================================================
...
scope_subnets AS (
  ...
  FROM SUBNETS s
  LEFT JOIN igw_route_tables igw
...
-- ============================================================================
-- In-Use Insecure NSGs on VNICs in Private/Non-Public Scope
-- Returns insecure NSG rules (ALL ports - TCP/22 - TCP/3389) attached to VNICs in subnets outside public-scope definition.
-- ============================================================================
WITH igw_route_tables AS (
  SELECT DISTINCT
    json_value(r.data, '$."identifier"' RETURNING VARCHAR2(200)) AS route_table_id
...
JOIN scope_subnets ps
  ON ps.subnet_id = vd.subnet_id
ORDER BY vd.vnic_id, ina.nsg_id, ina.rule_id, ina.insecure_case;


-- ============================================================================
-- Private/Non-Public Scope Insecure VNICs
-- Returns VNICs in private/non-public-scope subnets exposed by insecure security lists and/or insecure NSGs.
...
-- ============================================================================
-- Private/Non-Public Scope Insecure Subnets
-- Returns private/non-public-scope subnets exposed by insecure security lists.
-- (ALL ports - TCP/22 - TCP/3389)
-- ============================================================================
WITH igw_route_tables AS (
  SELECT DISTINCT
...
-- ============================================================================
-- PART B - Other Exposed Ports/Protocols in Private/Non-Public Scope
-- Scope: private/non-public-scope networks only, excluding ALL ports, TCP/22 and TCP/3389.
-- ============================================================================

-- ============================================================================
-- Summary of services used in private/non-public-scope subnets and vnics
-- Returns protocol+service exposure summary in private/non-public-scope networks, grouped by SECURITY_LIST or NSG.
-- Includes only:
--   - security lists effectively in use by private/non-public-scope subnets
--   - NSGs effectively in use by VNICs in private/non-public-scope subnets
...
-- ============================================================================
-- In-Use Other Security Lists in Private/Non-Public Scope
-- Returns security list rules in use by private/non-public-scope subnets excluding ALL ports, TCP/22 or TCP/3389.
-- ============================================================================
WITH igw_route_tables AS (
  SELECT DISTINCT
    json_value(r.data, '$."identifier"' RETURNING VARCHAR2(200)) AS route_table_id
...
-- ============================================================================
-- In-Use Other NSGs on VNICs in Private/Non-Public Scope
-- Returns NSG rules in use by VNICs in private/non-public-scope subnets excluding ALL ports, TCP/22 or TCP/3389.
-- ============================================================================
WITH igw_route_tables AS (
  SELECT DISTINCT
    json_value(r.data, '$."identifier"' RETURNING VARCHAR2(200)) AS route_table_id
...
-- ============================================================================
-- Private/Non-Public Scope VNICs Exposed by Other Ports/Protocols
-- Returns VNICs in private/non-public-scope subnets exposed by security lists and/or NSG rules excluding ALL ports, TCP/22 or TCP/3389.
-- ============================================================================
WITH igw_route_tables AS (
  SELECT DISTINCT
    json_value(r.data, '$."identifier"' RETURNING VARCHAR2(200)) AS route_table_id
...
-- ============================================================================
-- Private/Non-Public Scope Subnets Exposed by Other Ports/Protocols
-- Returns private/non-public-scope subnets exposed by security lists and/or NSG rules excluding ALL ports, TCP/22 or TCP/3389.
-- ============================================================================
WITH igw_route_tables AS (
  SELECT DISTINCT
    json_value(r.data, '$."identifier"' RETURNING VARCHAR2(200)) AS route_table_id
